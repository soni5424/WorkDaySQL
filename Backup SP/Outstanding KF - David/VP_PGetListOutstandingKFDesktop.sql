...
/****** Object:  StoredProcedure [dbo].[VP_PGetListOutstandingKFDesktop]    Script Date: 09/06/2020 08.26.15 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[VP_PGetListOutstandingKFDesktop] 
	@VendorID	varchar(20)
AS
BEGIN
...
SELECT DocNo,DocDate,DocStatus,AgrNo,AgrDesc,OutstandingAmount,Urutan,
KodeVendor,NamaVendor,ISNULL(A.NoMember,'') NoMemberPOS,ISNULL(B.CustID_KUNNR,'') NoMemberSAP
FROM(
select *,'1' Urutan from 
(
select A.VendorID KodeVendor,ISNULL(B.Name1,'')+ISNULL(' '+B.SORTL,'')NamaVendor,DocNo,DocDate,
...
OutstandingAmount
 from fi_amountkf A join SAP_Vendor B on B.Vendor_AC_Numb=A.VendorID
...
select A.Vendor_AC_Numb KodeVendor,
ISNULL(D.Name1,'')+ISNULL(' '+D.SORTL,'')NamaVendor,
C.NoFaktur DocNo,C.Tanggal DocDate,'FK Seragam' DocStatus, C.NoFaktur AgrNo,C.KeteranganSO AgrDesc,C.TotalFaktur-ISNULL(C.TotalBayar,0)-ISNULL(C.TotalPelunasan,0) OutstandingAmount,'2' Urutan 
from fi_mappingvendorcustpos A, fi_fkseragam C,
SAP_Vendor D
where A.NoMember=C.NoMember
and D.Vendor_AC_Numb = A.Vendor_AC_Numb 
and (C.TotalFaktur-ISNULL(C.TotalBayar,0)-ISNULL(C.TotalPelunasan,0))>0
--and A.vendor_ac_numb=@VendorID
union

select B.Vendor_AC_Numb KodeVendor,
ISNULL(B.Name1,'')+ISNULL(' '+B.SORTL,'')NamaVendor,
--ISNULL(D.NoMember,'') NoMemberPOS,
--ISNULL((SELECT CustId_KUNNR  from MasterMember E where E.NoMember=D.NoMember),'')NoMemberSAP,
A.ContractNo DocNo,A.ContractDate DocDate,'SO Contract' DocStatus,A.ContractNo AgrNo,C.Description AgrDesc,C.Amount OutstandingAmount,'3' Urutan from fi_SOContract A join SAP_Vendor B on A.Partner=B.Customer_Num
join fi_SOContractDetail C on C.ContractNo = A.ContractNo
left join FI_MappingVendorCustPOS D on D.Vendor_AC_Numb=B.Vendor_AC_Numb
...
select C.Vendor_AC_Numb KodeVendor,ISNULL(C.Name1,'')+ISNULL(' '+C.SORTL,'')NamaVendor,
--ISNULL(D.NoMember,'') NoMemberPOS,
--ISNULL(E.CustId_KUNNR,'') NoMemberSAP,
BillingNo DocNo,PostingDate DocDate,'Billing Contract' DocStatus, A.ContractNo AgrNo,ContractDesc AgrDesc,Amount-(SELECT ISNULL(SUM(B.Amount),0)FROM fi_potbillingcontract B where B.Assignment=A.Assignment) OutstandingAmount,'4' Urutan from fi_billingcontract A
join FI_SOContract B on A.ContractNo=B.ContractNo 
join sap_vendor C on C.Customer_Num = B.Partner
left join FI_MappingVendorCustPOS D on D.Vendor_AC_Numb=C.Vendor_AC_Numb
left join MasterMember E on E.NoMember=D.NoMember
--where B.Partner=(select Customer_Num from sap_vendor where Vendor_AC_Numb=@VendorID)

)AllData 
left join FI_MappingVendorCustPOS A on A.Vendor_AC_Numb=AllData.KodeVendor
left join MasterMember B on B.NoMember=A.NoMember
where OutstandingAmount>0
...
SELECT *,@VendorID KodeVendor,(SELECT ISNULL(Name1,'')+' '+ISNULL(SORTL,'') from SAP_Vendor where vendor_ac_numb=@VendorID) NamaVendor,
(select ISNULL(NoMember,'') from fi_mappingvendorcustpos where vendor_ac_numb=@VendorID)NoMemberPOS,
(select ISNULL(CustID_KUNNR,'') from fi_mappingvendorcustpos a join MasterMember b on a.NoMember=b.NoMember where a.vendor_ac_numb=@VendorID )NoMemberSAP
