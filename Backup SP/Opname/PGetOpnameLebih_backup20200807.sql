...
/****** Object:  StoredProcedure [dbo].[PGetOpnameLebih]    Script Date: 07/08/2020 14.02.41 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[PGetOpnameLebih]
    @PID varchar(50)
as
BEGIN
	...
	select article,batch,Unrestricted,Blocked,sum(qtyfisik) as qtyfisik,keterangan
	from 
	(
	...
	coalesce(qtyfisik,0) as qtyfisik,'' as keterangan from #Temp xxxx left join 
	(
	select xxx.article,xxx.batch,sum(qty) as Unrestricted from
	#Temp xxx where xxx.StockType='01' or xxx.StockType='06'	-- 5.12.15
	...
	group by xxx.article,xxx.batch			-- 5.12.15
	...
	where article != '' -- MM15.005.6.B 
	group by article,batch,Unrestricted,Blocked,keterangan
