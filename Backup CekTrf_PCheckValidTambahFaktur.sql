...
/****** Object:  StoredProcedure [dbo].[CekTrf_PCheckValidTambahFaktur]    Script Date: 22/07/2023 18.06.04 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[CekTrf_PCheckValidTambahFaktur]
	@NoFaktur	varchar(18),
	@Status		varchar(50),
...
BEGIN
	...
	Begin
		--if not exists (select 1 from TrxFakturBayar where NoFaktur=@NoFaktur and KodeJenisPembayaran='TRF')
		--	select 'No Faktur Tidak Mengandung Tipe Bayar Transfer'
		if( not exists(select 0 from TrxFakturBayar where NoFaktur = @NoFaktur and KodeJenisPembayaran in(select nilai from masterpilihan where grup = 'KodejenisPembayaranCekTransfer' )))	--01/07/2020
